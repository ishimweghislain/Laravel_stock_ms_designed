...
@section('content')
    <div class="flex justify-between items-center mb-6">
        <h1 class="text-3xl font-semibold text-gray-800 dark:text-white">Edit Product</h1>
    ...
    <div class="bg-white dark:bg-gray-800 rounded-lg shadow-md overflow-hidden">
        <form action="{{ route('products.update', $product) }}" method="POST" class="p-6">
            ...
            <div class="mb-6">
                ...
                <input type="text" name="pname" id="pname" value="{{ old('pname', $product->pname) }}" required
            ...
            <div class="mb-6">
                ...
                <input type="text" name="pcode" id="pcode" value="{{ old('pcode', $product->pcode) }}" required
                       class="w-full px-3 py-2 border border-gray-300 dark:border-gray-700 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:text-white">
                @error('pcode')
                    <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                @enderror
