...
@section('content')
    <div class="flex justify-between items-center mb-6">
        <h1 class="text-3xl font-semibold text-gray-800 dark:text-white">Add Stock In</h1>
    ...
    <div class="bg-white dark:bg-gray-800 rounded-lg shadow-md overflow-hidden">
        <form action="{{ route('stocki.store') }}" method="POST" class="p-6" id="stockiForm">
            @csrf
            ...
            <div class="mb-6">
                ...
                <input type="date" name="stockidate" id="stockidate" value="{{ old('stockidate', date('Y-m-d')) }}" required
                       class="w-full px-3 py-2 border border-gray-300 dark:border-gray-700 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:text-white">
                @error('stockidate')
                    <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                @enderror
            ...
            <div class="mb-6">
                ...
                <input type="number" name="unitprice" id="unitprice" value="{{ old('unitprice') }}" required min="0" step="0.01"
                       class="w-full px-3 py-2 border border-gray-300 dark:border-gray-700 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:text-white">
                @error('unitprice')
                    <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                @enderror
            ...
            <div class="mb-6">
                ...
                <input type="number" name="totalprice" id="totalprice" value="{{ old('totalprice') }}" required min="0" step="0.01" readonly
