...
@section('content')
    <div class="flex justify-between items-center mb-6">
        <h1 class="text-3xl font-semibold text-gray-800 dark:text-white">Add Stock Out</h1>
    ...
    <div class="bg-white dark:bg-gray-800 rounded-lg shadow-md overflow-hidden">
        <form action="{{ route('stocko.store') }}" method="POST" class="p-6" id="stockoForm">
            @csrf
            ...
            <div class="mb-6">
                ...
                <input type="date" name="stockodate" id="stockodate" value="{{ old('stockodate', date('Y-m-d')) }}" required
                       class="w-full px-3 py-2 border border-gray-300 dark:border-gray-700 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:text-white">
                @error('stockodate')
                    <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                @enderror
